...
main:
		...
		MOV		R5, #5			@ Number of cycles

cycle_loop:
        MOV     R4, #0             @ R4 = counter (start at 0)


count_loop:

        MOV     R0, R4             @ Put current number into R0 (argument for disp_num)
        BL      disp_num           @ Display number on LEDs

        MOV     R0, #100           @ Delay value in milliseconds
        BL      wait               @ Call wait function

        ADD     R4, R4, #1         @ counter++

        CMP     R4, #1024          @ Have we reached 2^10 = 1024?
        BLT     count_loop         @ If not, continue counting

        SUB     R5, R5, #1         @ One cylce complete => decrement cycle counter
        CMP     R5, #0             @ Are there cycles left?
        BGT     cycle_loop         @ If yes, restart counting from 0
...
.word   0x10         @ bit 2 = GPIO4  
.word   0x80         @ bit 3 = GPIO7 
.word   0x20000      @ bit 4 = GPIO17
.word   0x40000      @ bit 5 = GPIO18
.word   0x8000000    @ bit 6 = GPIO27
.word   0x400000     @ bit 7 = GPIO22
.word   0x800000     @ bit 8 = GPIO23
.word   0x1000000    @ bit 9 = GPIO24
