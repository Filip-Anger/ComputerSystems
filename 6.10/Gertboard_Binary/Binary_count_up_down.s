...
main:
        ...
        @ wait for keypress
        BL      read        @ read key into R0

        CMP     R0, #'1'         @ compare with character 1
        BEQ     count_up_mode    @ if 1  go up
        B       count_down_mode  @ otherwise  go down
...
@ COUNT DOWN (1023 to 0)
count_down_mode:

        MOV     R5, #3           @ number of cycles

down_cycle:
        MOV     R4, #1023        @ start at max (2^10 - 1)
