...
.equ GERT17, 17 		@ RPi GPIO to gertboard mappings
...
main:                               
		...
		BLT		exit				@	<0 (error) then exit
                ...
                @ Setup PWM led GERT17
                LDR		R0, =GERT17				@ Pin number
		MOV		R1, #1				@ Code for output
		BL		set_pin_function	@ Set pin to output
		CMP		R0, #0				@ If return value ... 
		BLT		exit				@	<0 (error) then exit
		...
		BLT		exit				@	<0 (error) then exit
                ...
                @ Setup PWM values
                MOV R6, #300    @ GERT22 (dim)
                MOV R8, #700    @ GERT17 (bright)
                MOV R9, #1024   @ total period

                MOV     R4, #300000         @ number of PWM cycles
...
pwm_loop:
                ...
                MOV     R0, R6           @ wait on_time 300 microsec
                BL      wait


                @ PWM LED OFF GERT22 (dim led)
                LDR     R0, =GERT22
                LDR     R1, =GPCLR0
                BL      set_pin_value

                @ Bright light stays on longer

                SUB R0, R8, R6        @ 700 - 300 = 400 microsec
                BL  wait
                ...
                @ remaining off time
                SUB R0, R9, R8        @ 1000 - 700 = 300 microsec
                BL      wait
