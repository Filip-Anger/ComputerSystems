...
next_guess:
        ...
        CMP   R10, R8           @ Was it correct?
        BEQ   extra_game        @ YES -> Ask to play again
...
@         MOV 	R1, #STDIN              @ TASK: Load 0 into R1 (time zone)
@         MOV		R7, #SYS_GETTIME             @ TASK: Place system call number for gettimeofday in R7
@         SWI 0                        @ TASK: Make the system call
@         LDR		R2, =musecs         @ TASK: Load a register with address of musecs variable
...
play_prompt:     .asciz "Play again? (y/n): "
.equ          play_prompt_len, 19

@@@@ Variables
.align
input:             .space 4             	@ TASK: Create user guess variable here (input buffer)
.align
